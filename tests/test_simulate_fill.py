...
from fillsim import Config, FillEvent, Leg, Spread, simulate_fill
...
def test_candidate_without_expiry_raises():
    cand = Spread(
        short=Leg(440, 0, 0), long=Leg(435, 0, 0),
        limit_credit=0.40, width=5.0, expiry=None,
    )
...
def test_first_fill_wins_even_when_a_higher_ev_candidate_also_crosses_same_bar():
    ...
    snap = make_snapshot([
        (EXPIRY, 440, 1.30, 1.30), (EXPIRY, 435, 0.95, 0.97),  # a crosses
        (EXPIRY, 450, 2.00, 2.00), (EXPIRY, 445, 1.35, 1.37),  # b crosses
    ])
...
def test_same_bar_tiebreak_is_deterministic_across_runs():
    ...
    snap = make_snapshot([
        (EXPIRY, 440, 1.30, 1.30), (EXPIRY, 435, 0.95, 0.97),
        (EXPIRY, 450, 2.00, 2.00), (EXPIRY, 445, 1.35, 1.37),
    ])
    r1 = simulate_fill(BAR_TS, snap, [a, b])
...
def test_multi_expiry_same_bar_both_cross_one_wins():
    ...
    snap = {
        (exp_a, 440): (1.30, 1.30), (exp_a, 435): (0.95, 0.97),
        (exp_b, 450): (2.00, 2.00), (exp_b, 445): (1.35, 1.37),
    }
...
def test_near_misses_count_per_candidate_per_bar_not_per_bar():
    ...
    snap = {
        (EXPIRY, 440): (1.25, 1.30), (EXPIRY, 435): (0.80, 0.85),  # 1.25 - 0.85 = 0.40
        (EXPIRY, 442): (1.30, 1.35), (EXPIRY, 437): (0.85, 0.90),  # 1.30 - 0.90 = 0.40
        (EXPIRY, 444): (1.35, 1.40), (EXPIRY, 439): (0.90, 0.95),  # 1.35 - 0.95 = 0.40
    }
